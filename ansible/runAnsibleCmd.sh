...
function usage {
    cat <<EOF
========================================================================================
Usage: ${SCRIPTNAME} -c <config_file> -p <host_pattern> -a <ansible_cmd> <[-l <logdir>] [-h] 
where
    <config_file>  Name with full path of the configuration file
    <host_pattern> Ansible host pattern to match
    <ansible_cmd>  Command to run on the target hosts
...
while getopts ":c:p:a:l:h" opt
do
    case $opt in
        c)  DL_CONFIG_FILE=${OPTARG}
            ;;
        p)  ANSIBLE_HOST=${OPTARG}
            ;;
        a)  ANSIBLE_CMD=${OPTARG}
            ;;
        l)  LOGDIR=${OPTARG}
            ;;
        h)  HELP_WANTED=1
...
if [ -z ${DL_CONFIG_FILE} ] || [ -z ${ANSIBLE_CMD} ] || [ -z ${ANSIBLE_HOST} ]; then
   usage
   exit 1
fi
...
ansible $ANSIBLE_HOST -i ${HOME_DIR}/azure_inv.sh --private-key ${CONFIG_DIR}/ssh_keys/id_rsa --extra-vars "@${DL_CONFIG_FILE}" -a "${ANSIBLE_CMD}" -u $admin_name
