...
DL_CONFIG_FILE=
# Compatibility for undefined hdinsight_type
hdinsight_type="spark"
HELP_WANTED=0
...
ADLS_NAME=$(echo ${main_res_group} | tr -d '-')adls
...
if [ "$hdinsight_type" = "domainjoined" ]; then
    executeAndLog "jinja2 $HOME_DIR/ddatalake.parameters.j2template.json $CONFIG_DIR/$CONFIG_FILE --strict -D admin_ssh_key=\"${ADMIN_SSH_KEY}\" -D ad_tenant_id=$AD_TENANT_ID -D sp_application_id=$SP_APP_ID -D sp_certificate_content=\"${SP_CERTIFICATE}\" -D adls_name=$ADLS_NAME > $CONFIG_DIR/$workspace_name.parameters.json"
else
...
if [ -f "${CONFIG_DIR}"/"${workspace_name}"_pre_install.sh ]; then
    . "${CONFIG_DIR}"/"${workspace_name}"_pre_install.sh
fi


if [ "$hdinsight_type" = "rserver" ]; then
    executeAndLog "echo \"Creating Azure HDInsight RServer for Data Lake...\""
    executeAndLog "az group deployment create -n ${res_group}_$workspace_name -g $res_group --template-file \"$HOME_DIR/rdatalake.resources.json\" --parameters \"@${CONFIG_DIR}/$workspace_name.parameters.json\"  --verbose"
elif [ "$hdinsight_type" = "domainjoined" ]; then
    executeAndLog "echo \"Creating Azure HDInsight Domain Joined for Data Lake...\""
    executeAndLog "az group deployment create -n ${res_group}_$workspace_name -g $res_group --template-file \"$HOME_DIR/ddatalake.resources.json\" --parameters \"@${CONFIG_DIR}/$workspace_name.parameters.json\"  --verbose"
else
    executeAndLog "echo \"Creating Azure HDInsight Spark for Data Lake...\""
...
if [ -f "${CONFIG_DIR}"/"${workspace_name}"_post_install.sh ]; then
    . "${CONFIG_DIR}"/"${workspace_name}"_post_install.sh
fi
