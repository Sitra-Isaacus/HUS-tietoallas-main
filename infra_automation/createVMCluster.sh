...
executeAndLog "jinja2 $HOME_DIR/vmcluster.parameters.j2template.json $CONFIG_DIR/$CONFIG_FILE --strict -D admin_ssh_key=\"${ADMIN_SSH_KEY}\" > $CONFIG_DIR/$cluster_name.parameters.json"

executeAndLog "echo \"Creating Cluster...\""
executeAndLog "az group deployment create -n ${res_group}_$cluster_name -g $res_group --template-file \"$HOME_DIR/vmcluster.resources.json\" --parameters \"@$CONFIG_DIR/$cluster_name.parameters.json\""
