...
executeAndLog "jinja2 $HOME_DIR/integration.parameters.j2template.json $CONFIG_DIR/$CONFIG_FILE --strict -D admin_ssh_key=\"${ADMIN_SSH_KEY}\" > $CONFIG_DIR/integration.parameters.json"
...
executeAndLog "az group deployment create -n ${res_group}_main -g $res_group --template-file \"$HOME_DIR/integration.resources.json\" --parameters \"@$CONFIG_DIR/integration.parameters.json\" --verbose"
