...
function create_keyvault {
    ...
    executeAndLog "az keyvault create --enabled-for-deployment --enabled-for-disk-encryption --enabled-for-template-deployment -n $KEYVAULT_NAME -g $KEYVAULT_RES_GROUP -l $location"
}
...
ADLS_NAME=$(echo ${res_group} | tr -d '-')adls
...
get_adls_keyvault_sp_id ${ADLS_NAME}
